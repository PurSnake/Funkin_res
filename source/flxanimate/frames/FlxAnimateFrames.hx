...
class FlxAnimateFrames extends FlxAtlasFrames
{
	...
	public static function fromTextureAtlas(Path:String):FlxAtlasFrames
	{
		...
		else
		{
			...
			{
				var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(Utils.getText('$Path/spritemap.json'), String.fromCharCode(0xFEFF), ""));
			...
			{
				var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(Utils.getText('$Path/spritemap$i.json'), String.fromCharCode(0xFEFF), ""));
				var curSpritemap = Utils.getBitmapData('$Path/${curJson.meta.image}');
				if (curSpritemap != null)
				{
					var graphic = FlxG.bitmap.add(curSpritemap, '$Path/${curJson.meta.image}');
