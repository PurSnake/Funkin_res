...
class PauseSubState extends MusicBeatSubState
{
	...
	{
		var pauseMusicPath = Paths.music('breakfast$musicSuffix/breakfast$musicSuffix');
	...
	function transitionIn():Void
	{
		...
		for (child in metadata.members)
		{
			...
			FlxTween.tween(child, {alpha: (child.alpha = 0) + 1, y: child.y + 5}, 1.8, {ease: FlxEase.quartOut, startDelay: delay});
			delay += 0.1;
		}
	...
	{
		currentEntry = funkin.util.MathUtil.curSelectionWrap(currentEntry, change, currentMenuEntries);
