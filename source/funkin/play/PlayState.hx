...
class PlayState extends MusicBeatSubState
{
	...
	**/
	...
	public override function update(elapsed:Float):Void
	{
		...
		if (!isInCutscene && !disableKeys)
		{
			...
			if (health <= Constants.HEALTH_MIN && !isPracticeMode && !isPlayerDying)
			{
				...
				deathPreTransitionDelay > 0 ? new FlxTimer().start(deathPreTransitionDelay, (_) -> moveToGameOver())
				: moveToGameOver();
	...
	dynamic function initHealthBar():Void
	{
		...
		for (obj in [healthBar, healthBarBG, scoreText]) obj.camera = camHUD;
	}
	...
	function initCharacters():Void
	{
		...
		if (girlfriend != null)
			...
			trace("Chosen GF was '' so we don't load one.");
	...
	dynamic function initStrumlines():Void
	{
		...
		var styleOffsets = noteStyle.getStyleOffset();
		// Position the player strumline on the right half of the screen
		playerStrumline.x = FlxG.width / 2 + Constants.STRUMLINE_X_OFFSET + styleOffsets[0]; // Classic style
		// playerStrumline.x = FlxG.width - playerStrumline.width - Constants.STRUMLINE_X_OFFSET; // Centered style
		playerStrumline.y = Preferences.downscroll ? FlxG.height - playerStrumline.height - Constants.STRUMLINE_Y_OFFSET - styleOffsets[1] : Constants.STRUMLINE_Y_OFFSET + styleOffsets[1];
		playerStrumline.zIndex = 1001;
		playerStrumline.cameras = [camHUD];

		// Position the opponent strumline on the left half of the screen
		opponentStrumline.x = Constants.STRUMLINE_X_OFFSET + styleOffsets[0];
		opponentStrumline.y = Preferences.downscroll ? FlxG.height - opponentStrumline.height - Constants.STRUMLINE_Y_OFFSET - styleOffsets[1]: Constants.STRUMLINE_Y_OFFSET + styleOffsets[1];
	...
	{
		scoreText.text = isBotPlayMode ? 'Bot Play Enabled' : 'Score:' + songScore;
	}
	...
	function goodNoteHit(note:NoteSprite, input:PreciseInputEvent):Void
	{
		...
		var event:HitNoteScriptEvent = new HitNoteScriptEvent(note, healthChange, score, daRating, isComboBreak, Highscore.tallies.combo + 1, noteDiff, daRating == 'sick');
		dispatchEvent(event);
	...
	public function endSong(rightGoddamnNow:Bool = false):Void
	{
		...
		if (PlayStatePlaylist.isStoryMode)
		{
			...
			if (targetSongId == null)
			{
				...
				isSubState ? this.close()
				: rightGoddamnNow ? moveToResultsScreen(isNewHighscore, prevScoreData) : zoomIntoResultsScreen(isNewHighscore, prevScoreData);
			}
			else
			{
	 ...
	 */
	dynamic function zoomIntoResultsScreen(isNewHighscore:Bool, ?prevScoreData:SaveScoreData):Void
	{
	...
	public function tweenCameraZoom(?zoom:Float, ?duration:Float, ?direct:Bool, ?ease:Null<Float->Float>):Void
	{
		...
		duration == 0 ? currentCameraZoom = targetZoom : cameraZoomTween = FlxTween.tween(this, {currentCameraZoom: targetZoom}, duration, {ease: ease});
	}
