...
class ChartEditorNotificationHandler
{
			...
			?actions:Array<NotificationActionData>):Notification
	{
		...
		final notifSound = FlxG.sound.load(Paths.sound("notif"));
		notifSound.onComplete = __soundOnComplete.bind(notifSound);
		notifSound.play();
		FlxG.sound.list.remove(notifSound); //due to stateSwitch bug
